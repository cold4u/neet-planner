...
function convertLaTeXTabularToHTML(text) {
  if (!text || typeof text !== 'string') return text;
  // Match nested or single \begin{tabular}{...} ... \end{tabular}
  const tabularRegex = /\\begin\{tabular\}\s*\{[^}]*?\}([\s\S]*?)\\end\{tabular\}/g;

  return text.replace(tabularRegex, function(match, content) {
    let cleaned = content.replace(/\\hline/g, '');
    cleaned = cleaned.replace(/\\multicolumn\{\d+\}\{[^}]*\}\{([^}]*)\}/g, '$1');
    cleaned = cleaned.replace(/\\begin\{tabular\}\{[^}]*\}|\s*\\end\{tabular\}/g, '');

    const rows = cleaned.split(/\\\\/).map(r => r.trim()).filter(Boolean);
    if (rows.length === 0) return '';

    let htmlTable = '<div style="overflow-x:auto; margin:12px 0;"><table class="rendered-latex-table" style="width:100%; border-collapse:collapse; font-size:13px; text-align:left; border:1px solid rgba(255,255,255,0.15); background:rgba(15, 23, 42, 0.7); border-radius:6px; overflow:hidden;">';
    ...
    rows.forEach((row, index) => {
      ...
      if (cells.length > 0 && cells.some(c => c.length > 0)) {
        ...
        const style = isHeader ? 'padding:8px 12px; font-weight:700; border:1px solid rgba(255,255,255,0.15); background:rgba(0, 212, 170, 0.15); color:var(--primary, #00d4aa);' : 'padding:8px 12px; border:1px solid rgba(255,255,255,0.1); color:var(--text-primary, #f8fafc);';
        
        htmlTable += '<tr>';
        cells.forEach(cell => {
          htmlTable += `<${tag} style="${style}">${cell}</${tag}>`;
        });
        htmlTable += '</tr>';
      }
    });
...
function parseMarkdownAndKaTeX(text) {
  ...
  // 0. Parse LaTeX tabular environments into HTML tables first
  html = convertLaTeXTabularToHTML(html);
