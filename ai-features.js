/**
 * SHUBxCOLD NEET 2027 Planner — AI Features Engine (BYOK + Gemini 2.0 Flash)
 * 
 * Features:
 * 1. BYOK Gemini 2.0 Flash API wrapper with Exponential Backoff on 429
 * 2. AI Tutor (Doubt Solver with Subject Modes & KaTeX)
 * 3. AI CBT Mock Test Generator & NTA-Style Exam Simulator
 * 4. PDF Question Extractor → NEET Test Builder (using PDF.js)
 * 5. AI Study Optimizer Widget ("What Should I Study?")
 * 6. AI Error Pattern Analyzer ("Analyze My Mistakes" & Recovery Quiz)
 * 7. NEET News & NTA Official Updates Hub + AI Summarizer
 */

// Model Configuration — STRICTLY gemini-2.0-flash
...
// Central BYOK Gemini API Caller with Exponential Backoff on HTTP 429
async function callGeminiAPI(prompt, systemInstruction = "", onStatus = null) {
  ...
  const endpoint = `${GEMINI_API_BASE}/${GEMINI_MODEL}:generateContent?key=${apiKey.trim()}`;
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        if (attempt <= backoffDelays.length) {
          const delaySec = backoffDelays[attempt - 1] / 1000;
          if (onStatus) onStatus(`Google Free Tier rate limit reached. Auto-retrying in ${delaySec}s...`);
      ...
      if (err.message === "NO_API_KEY" || err.message === "HTTP_429_EXCEEDED") {
        throw err;
      }
      if (attempt < backoffDelays.length && err.message.includes("429")) {
        attempt++;
        const delaySec = backoffDelays[attempt - 1] / 1000;
        if (onStatus) onStatus(`Rate limit hit. Retrying in ${delaySec}s...`);
        await sleep(backoffDelays[attempt - 1]);
      } else {
        throw err;
      }
...
async function testApiKeyConnection() {
  ...
  try {
    const reply = await callGeminiAPI("Respond with only the word: CONNECTED");
    if (reply && reply.includes("CONNECTED")) {
      if (statusBadge) {
        statusBadge.innerHTML = `<span style="color:#00d4aa; font-weight:bold;">🟢 Connected (Gemini 2.0 Flash Ready)</span>`;
      }
      alert("🎉 Connection Successful! Gemini 2.0 Flash is ready for all AI features.");
    } else {
      if (statusBadge) {
    ...
    if (statusBadge) {
      statusBadge.innerHTML = `<span style="color:#ef4444; font-weight:bold;">🔴 Connection Failed: ${err.message}</span>`;
    }
  ...
  if (statusCard) {
    ...
    statusCard.innerHTML = `
      <div class="glass-card" style="text-align:center; padding:20px;">
        ...
        <h4>Generating ${numQuestions} NEET-pattern MCQs using Gemini 2.0 Flash...</h4>
  ...
  if (statusCard) {
    ...
    statusCard.innerHTML = `
      <div class="glass-card" style="text-align:center; padding:20px;">
        ...
        <p style="font-size:12px; color:#aaa;">Extracted text will be parsed into NEET MCQs using Gemini 2.0 Flash...</p>
      </div>
    `;
  }
...
${pdfText.substring(0, 8000)}`;
    ...
    const rawResult = await callGeminiAPI(prompt, "You are a PDF question extractor. Output JSON array only.");
...
async function analyzeMistakesWithAI() {
  ...
  resultContainer.innerHTML = `
    <div class="glass-card" style="padding:15px; text-align:center;">
      <div class="spinner" style="margin:0 auto 10px auto;"></div>
      Analyzing your mistake patterns using Gemini 2.0 Flash...
    </div>
  `;
...
async function summarizeNews(title) {
  ...
  alert(`⚡ Summarizing "${title}" using Gemini 2.0 Flash...`);
  try {
