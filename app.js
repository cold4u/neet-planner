...
function togglePomoTimer() {
  ...
  try {
    ...
    if (inFocus && inShort && inLong) {
      ...
      const l = parseInt(inLong.value, 10);
      if (!isNaN(f) && !isNaN(s) && !isNaN(l)) {
        const curF = pomoV3DurationMap.focus / 60;
        const curS = pomoV3DurationMap.short / 60;
        const curL = pomoV3DurationMap.long / 60;
        if (f !== curF || s !== curS || l !== curL) {
          savePomoSettings(true);
        }
      }
    }
  } catch(e) {}
...
function savePomoSettings(silent) {
  ...
  if (isNaN(focusVal) || focusVal < 1 || focusVal > 180) {
    if (!silent) showToast("⚠️ Focus time must be between 1 and 180 minutes.");
    return;
  }
  if (isNaN(shortVal) || shortVal < 1 || shortVal > 180) {
    if (!silent) showToast("⚠️ Short break must be between 1 and 180 minutes.");
    return;
  }
  if (isNaN(longVal) || longVal < 1 || longVal > 180) {
    if (!silent) showToast("⚠️ Long break must be between 1 and 180 minutes.");
    return;
  }
  ...
  if (!silent) showToast("⚙️ Pomodoro durations updated!");
}
