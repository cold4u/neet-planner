          ...
          properties: {
            questions: {
              ...
              items: {
                ...
                required: ["question", "options", "correct_option_idx", "explanation", "concept"]
              }
            }
          },
          required: ["questions"]
...
function buildCbtReview() {
  ...
  cbtQuestions.forEach((q, qIdx) => {
    ...
    const conceptBadgeHtml = q.concept ? `<span style="background:rgba(255,215,0,0.1); color:#FFD700; border:1px solid rgba(255,215,0,0.25); padding:2px 8px; border-radius:12px; font-size:11px; font-weight:600;">📌 ${q.concept}</span>` : '';
  ...
  cbtResults.forEach((res) => {
    ...
    const headerHtml = `
      <div style="padding:16px 20px; display:flex; justify-content:space-between; align-items:center; cursor:pointer; flex-wrap:wrap; gap:12px; background:rgba(255,255,255,0.01);" onclick="toggleCbtCardExpand('${res.id}')">
        ...
        <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
          ...
          <button class="btn btn-secondary" onclick="event.stopPropagation(); deleteCbtResult('${res.id}')" style="padding:4px 10px; font-size:12px; border-color:rgba(255,107,107,0.3); color:var(--tertiary);" title="Delete Record">🗑️ Delete</button>
        </div>
      </div>
    `;
    ...
    if (res.questions && Array.isArray(res.questions)) {
      res.questions.forEach((q, qIdx) => {
        ...
        questionsReviewHtml += `
          <div class="review-question-card" style="background:rgba(255, 255, 255, 0.02); border:1px solid rgba(255, 255, 255, 0.06); border-radius:12px; padding:16px; margin-bottom:12px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; flex-wrap:wrap; gap:8px;">
              <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                <span style="font-weight:700; font-family:'Share Tech',sans-serif; color:#FFD700;">Question ${qIdx + 1}</span>
                ${conceptBadgeHtml}
            ...
            <div style="font-size:14px; font-weight:600; margin-bottom:12px; line-height:1.5; color:#fff;">${q.question}</div>
            <div style="margin-bottom:14px;">${optionsHtml}</div>
            <div style="background:rgba(255,215,0,0.03); border-left:3px solid #FFD700; padding:10px 14px; border-radius:0 8px 8px 0; font-size:12px; line-height:1.5;">
              <strong style="color:#FFD700; display:block; margin-bottom:4px; font-family:'Share Tech',sans-serif;">Explanation:</strong>
              <span style="color:#e5e7eb;">${q.explanation}</span>
            </div>
          </div>
